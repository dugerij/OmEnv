...
def _bbox(image, cnts, ax):
    ...
    for c in cnts:
        ...
        ax.add_patch(Rectangle((x, y), w, h, colour = 'red', fill=False))
...
def _drawMask(image, cnts, fill=True):
    ...
    heatmap_imh = cv2.applyColorMap(image, cv2.COLORMAP_JET)
    t = 2
    if fill:
        t = -1
...
def main():
    st.set_page_config(page_title='EnvisionIt', page_icon=None, layout='centered', initial_sidebar_state='auto')
    st.title('Dectecting pathologies through Computer vision in Ultrasound')
