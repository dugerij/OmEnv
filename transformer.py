...
class SelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        ...
        # keys shape: (N, key_len, heads, head_diim)
        ...
        out = torch.einsum("nhqx, nxhd -> nqhd", [attention, values]).reshape(
            N, query_len, self.heads*self.head_dim
            )
        #  attention shape : (N, heads, query_len, key_len)
        #  values shape: (N, value_len, heads, head_dim)
        #  out shape : (N, query_len, head, head_dim) then flatten last two dimentions
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_size, heads, dropout, foreward_expansion):
        ...
        self.feed_forward = nn.Sequential(
            nn.Linear(embed_size, foreward_expansion*embed_size),
            nn.ReLU(),
            nn.Linear(foreward_expansion*embed_size, embed_size)
        )
        ...
        self.layers = nn.ModuleList(
            [
                TransformerBlock(
                    ...
                    heads, 
                    dropout=dropout, 
                    foreward_expansion=foreward_expansion
                    )
